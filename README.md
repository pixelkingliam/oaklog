...
C# Customizable Flexible Logger.

Lightweight and customizable logger for .NET 6+, made to be easy to setup.

# Installation

Install the [Pixel.Oaklog package on nuget.org](https://www.nuget.org/packages/Pixel.OakLog).
...
LInclude the `Pixel.OakLog` namespace
...
Using the OLog.Create(string, (int, int, int)) function you can quickly create an OLog which by default, will print to the console, the rgb value of the tuple is for the background color of the LogTypes.

`OLog MyLogger = OLog.Create("MYLOGTYPE", (119,136,153));`



`MyLogger.Print("Hello, World")`

This will output [MYLOGTYPE][(time since startup in seconds)] Hello World


see **Customizing your output** for more information
